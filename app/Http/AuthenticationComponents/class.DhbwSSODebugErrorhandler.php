 ...
 *
 * This is the debugErrorhandler. It makes no redirects.
 ...
 */
class DhbwSSODebugErrorhandler extends DhbwSSOErrorhandler {
	...
	public function processError($key, $additional = "") {			
		if ($this->isUrl($key)) {
			$this->debug("ErrorMessage (".$key.") is an URL. Redirect would be set now", $this->errorMessages[$key]);
		}
		else {
			$this->debug("Error", $this->getErrorMessage());	
		}
		die();	
		return false;
	}
	 ...
	 *
	 * Returns all errorMessages for Debugging
	 * @return void
	 */			
	public function debugErrorMessages() {
		foreach($this->errorMessages as $key=>$value) {
			$this->debug("errorMessage '".$key."' set to", $value);
		}
	}
}
